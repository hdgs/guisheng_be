...
def main_page():
    ...
    if kind == 0:
        tolist = []
        for n in News.query.filter_by(published=1).filter_by(tea=0).filter_by(freshmen=0).order_by(News.time.desc()).limit(count+page*count):
            tolist.append(n)
        ...
        posts = post_kind.query.filter_by(published=1).filter_by(tea=0).filter_by(freshmen=0).order_by(post_kind.time.desc()).limit(count).offset((page)*count)
        return Response(json.dumps([{
                "kind":kind,
                "article_id":_post.id,
...
def list():
    ...
    if kind in [3,4]:
        ...
        posts = post_kind.query.filter_by(freshmen=0).order_by(post_kind.time.desc()).limit(count).offset((page-1)*count)
    return Response(json.dumps([{
            "kind":kind,
            "article_id":_post.id,
            "img_url":_post.img_url if _post.__class__!=Picture \
